<template>
    <div class="min-h-screen flex flex-col bg-j2aLightBlue text-gray-800">
      ...
      <header class="bg-j2aBlue text-white shadow-lg sticky top-0 z-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center">
          ...
          <!-- CORRECTION : Utilisation de cartStore.toggleCart() et cartStore.itemCount -->
            ...
            <div v-for="game in filteredGames" :key="game.id" class="bg-white rounded-2xl overflow-hidden card-shadow hover:scale-105 transition duration-300 border-2 border-transparent hover:border-j2aOrange flex flex-col">
              ...
              <div class="p-5 flex flex-col flex-1">
                ...
                <div class="mt-auto pt-4">
                  ...
                  <!-- CORRECTION : Utilisation de cartStore.addItem -->
  ...
  Ajouter au devis
      ...
      <!-- PANIER VOLANT -->
      <!-- CORRECTION : isCartOpen devient cartStore.isOpen -->
      <div v-if="cartStore.isOpen" class="fixed inset-0 z-50 overflow-hidden">
        <!-- CORRECTION : closeCart() au clic sur le fond -->
        ...
        <div class="fixed inset-y-0 right-0 max-w-md w-full flex">
          <div class="w-full bg-white shadow-2xl flex flex-col">
            <div class="px-6 py-4 bg-j2aBlue text-white flex items-center justify-between">
              ...
              <!-- CORRECTION : closeCart() au clic sur la croix -->
            ...
            <!-- Bouton de redirection (Fixé en bas) -->
...
// Nouvelle fonction d'ajout avec animation
// On crée la référence qui correspond au ref="cartButtonRef" du HTML
const cartButtonRef = ref(null);

// Nouvelle fonction d'ajout avec ciblage dynamique
const addToCart = (game, event) => {
  cartStore.addItem(game);

  // Calcul des coordonnées d'arrivée (le centre du bouton panier)
  let destinationX = window.innerWidth - 80; // Valeur par défaut
  let destinationY = 30; // Valeur par défaut
  
  if (cartButtonRef.value) {
    const rect = cartButtonRef.value.getBoundingClientRect();
    destinationX = rect.left + (rect.width / 2) - 12; // -12 pour centrer la balle de 24px (w-6)
  ...
  flyingItems.value.push({
    id,
    startX: event.clientX - 12, // On centre la balle sur le curseur
...
const goToCheckout = () => {
  cartStore.closeCart();
  router.push('/devis'); // redirection vers la page de validation
};
</script>